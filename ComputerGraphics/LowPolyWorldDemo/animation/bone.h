	...
	for (int i = 0; i < anim->mNumChannels; i++) {
		...
		if (ret != std::string::npos)
		{
			...
			if (checkAssimpFbx)
			{
				...
				for (int i = 0; i < (int)assimpFbxVector.size(); i++)
				{
...
public:
	...
	void setpos(Camera& camera, GLFWwindow* window, float height)
	{
		...
		//变大模型
		this->modelMatrix = glm::scale(this->modelMatrix, (float)2 * glm::vec3(2.0f, 2.0f, 2.0f));
		//向上轴混乱很常见。
...
//		Shader shader("bone.vs", "bone.fs");
	...
	void show(Shader& shader, Camera& camera, glm::vec3 lightPos, GLFWwindow* window, float height)
	{
		...
		float elapsedTime = (float)glfwGetTime() * 800;

		elapsedTime = (float)((int)elapsedTime % 850);
